class MindBoxApp {
    ...
    renderEntry(entry) {
        ...
        let timeNum = timeParts[0];
        const ampm = timeParts[1] || '';
        
        timeNum = timeNum.replace(/\d/g, d => this.toArabicNumber(d));
        
        return `
            <div class="entry-header">
                <span class="entry-time">🕒 ${timeNum} ${ampm}</span>
    ...
    async saveEntry() {
        ...
        try {
            ...
            console.error('خطأ في حفظ المدخلة:', error);
    ...
    async deleteEntry(entryId) {
        if (confirm('هل أنت متأكد من حذف هذه المدخلة؟')) {
            try {
                ...
                console.error('خطأ في حذف المدخلة:', error);
